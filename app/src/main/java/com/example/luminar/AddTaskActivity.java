...
public class AddTaskActivity extends AppCompatActivity {
    ...
    private void showColorPicker() {
        String[] colors = {"Green", "Blue", "Red", "Yellow", "Purple", "Orange"};
        String[] colorCodes = {"#4CAF50", "#2196F3", "#F44336", "#FFEB3B", "#9C27B0", "#FF9800"};
        int[] categoryIds = {1, 2, 3, 4, 5, 6};

        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Pick a Color/Category")
                .setItems(colors, (dialog, which) -> {
                    selectedColor = colorCodes[which];
                    selectedCategory = new Category(categoryIds[which], colors[which], colorCodes[which]);
                    edtPickColor.setText(colors[which]);
                    edtPickColor.setTextColor(Color.parseColor(colorCodes[which]));
                })
                .show();
    }
